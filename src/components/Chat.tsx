  ...
  return (
    <div className="mt-2 p-2 bg-accent/10 border border-accent/30 rounded-lg">
      ...
      {fileSources.length > 0 && (
        <div className="mb-2">
          <p className="text-xs text-accent mb-1">📁 Dari file ({fileSources.length}):</p>
          <div className="flex flex-wrap gap-1">
            {fileSources.map((source, index) => (
              <button
                key={index}
                onClick={() => onSourceClick(source)}
                className="inline-flex items-center gap-1 px-2 py-1 bg-accent/20 hover:bg-accent/30 text-accent rounded text-xs transition-colors"
              >
      ...
      {manualSources.length > 0 && (
        <div>
          <p className="text-xs text-accent mb-1">✏️ Manual ({manualSources.length}):</p>
          <div className="flex flex-wrap gap-1">
            {manualSources.map((source, index) => (
              <button
                key={index}
                onClick={() => onSourceClick(source)}
                className="inline-flex items-center gap-1 px-2 py-1 bg-secondary hover:bg-secondary/80 text-secondary-foreground rounded text-xs transition-colors"
              >
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col h-[650px] mx-auto border-1 rounded-md">
        ...
        <div className="flex justify-between items-center p-4 border-b bg-card rounded-t-lg">
          <div>
            <h2 className="text-xl font-semibold text-card-foreground">Chat dengan AI</h2>
                  ...
                  <div className="flex justify-between items-start">
                    <div className="flex-1">
                      ...
                      <CardDescription className="text-xs">
                        {source.source}
                        {source.file_info && (
                          <span className="ml-2 inline-flex items-center px-2 py-0.5 rounded text-xs bg-accent/20 text-accent">
